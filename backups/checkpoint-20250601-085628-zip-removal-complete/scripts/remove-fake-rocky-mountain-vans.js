...
console.log('🗑️  Removing fake Rocky Mountain Vans entry...');
...
// Find and remove Rocky Mountain Vans (ID 29)
const originalCount = buildersData.length;
const filteredBuilders = buildersData.filter(builder => builder.id !== 29);
...
if (buildersByState.Colorado) {
  ...
  buildersByState.Colorado = buildersByState.Colorado.filter(builder => builder.id !== 29);
...
console.log('\n✅ Fake Rocky Mountain Vans entry completely removed from both files');
console.log('📊 Colorado now has only verified, authentic builders');
