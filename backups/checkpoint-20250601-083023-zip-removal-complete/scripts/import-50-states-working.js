...
async function importAllStates() {
  try {
    ...
    // Try to read credentials from .env file directly
    let credentials;
    try {
      ...
      if (process.env.GOOGLE_SHEETS_CREDENTIALS) {
        ...
        console.log('📋 Data is ready for manual import via CSV file');
        console.log('   Use the file: 50-states-builders.csv');
        return;
      }
    } catch (error) {
      console.log('⚠️  Could not parse credentials, using manual import method');
      console.log('📋 Data is ready for manual import via CSV file');
      console.log('   Use the file: 50-states-builders.csv');
      return;
    }
    ...
    console.log('3. Upload the file: 50-states-builders.csv');
    console.log('4. Choose "Append to current sheet" or "Replace current sheet"');
...
importAllStates()
  ...
  .catch((error) => {
    console.log('\n📁 CSV file is ready for manual import: 50-states-builders.csv');
  });
