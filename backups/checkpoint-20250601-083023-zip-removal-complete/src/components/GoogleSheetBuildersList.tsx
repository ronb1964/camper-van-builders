...
const GoogleSheetBuildersList: React.FC<GoogleSheetBuildersListProps> = ({ 
  ...
  // Fetch builders data from the JSON data service
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Get all builders from JSON data service
        ...
        // Debug Advanced RV specifically
        const advancedRV = allBuilders.find(b => b.name === 'Advanced RV');
        if (advancedRV) {
          console.log('🔍 ADVANCED RV FOUND:', advancedRV.name);
          console.log('🖼️ ADVANCED RV GALLERY:', advancedRV.gallery);
          console.log('📍 ADVANCED RV LOCATION:', advancedRV.location);
        } else {
          console.log('❌ ADVANCED RV NOT FOUND');
        }
  ...
  const builders = searchedBuilderName ? filteredBuilders : (buildersByState[selectedState] || []);
