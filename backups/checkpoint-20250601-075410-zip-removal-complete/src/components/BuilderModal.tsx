...
const BuilderModal: React.FC<BuilderModalProps> = ({ builder, open, onClose }) => {
  ...
  useEffect(() => {
    if (builder?.gallery?.[0]) {
      setMainImage(builder.gallery[0].url);
    }
  }, [builder]);
  ...
  const validGalleryImages = (builder.gallery || []).filter(item => item && item.url && item.url.startsWith('http')).map(item => item.url);
          ...
          <Box>
            {(gallery || []).length > 0 ? (
              <Box>
                ...
                >
                  {(gallery || []).filter(item => item && item.url && item.url.startsWith('http')).map((item, index) => (
                    <Box
                      key={index}
