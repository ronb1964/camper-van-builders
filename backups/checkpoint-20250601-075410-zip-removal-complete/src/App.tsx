...
const App = () => {
  ...
  useEffect(() => {
    const fetchBuilders = async () => {
      try {
        ...
        // Force refresh to get latest data (including new gallery photos)
        await jsonDataService.forceRefresh();
  ...
  const handleBuilderNameSearch = useCallback((builderName: string) => {
    console.log('Builder name searched:', builderName);
    setSearchedBuilderName(builderName);
    setSelectedState(''); // Clear state selection when searching by name
    
    if (builderName.trim()) {
      // Search for builders by name across all states
      const allBuilders = Object.values(buildersByState).flat();
      const filtered = allBuilders.filter(builder => 
        builder.name.toLowerCase().includes(builderName.toLowerCase())
      );
      setFilteredBuilders(filtered);
      
      // Update map center based on filtered builders
      if (filtered.length > 0) {
        const totalLat = filtered.reduce((sum, b) => sum + b.location.lat, 0);
        const totalLng = filtered.reduce((sum, b) => sum + b.location.lng, 0);
        
        setMapCenter({
          lat: totalLat / filtered.length,
          lng: totalLng / filtered.length
        });
        
        console.log(`🔍 Found ${filtered.length} builders matching "${builderName}"`);
      }
    } else {
      setFilteredBuilders([]);
    }
  ...
  const handleClearResults = useCallback(() => {
    ...
    setMapCenter({ lat: 40.0583, lng: -74.4057 });
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Container maxWidth="xl" sx={{ py: 2, mb: 6 }}>
        ...
        {(selectedState || searchedBuilderName) && (
          <Box id="search-results">
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 4, mb: 2 }}>
              <Typography variant="h4" component="h3">
                {selectedState ? `Builders in ${selectedState}` : `Search Results for "${searchedBuilderName}"`}
              </Typography>
            ...
            {activeTab === 0 ? (
              ...
              <Box sx={{ height: '600px', borderRadius: 2, overflow: 'hidden' }}>
                ...
                >
                  ...
                  {(selectedState ? buildersByState[selectedState] : filteredBuilders).map((builder) => (
                    <Marker
                      key={builder.id}
                  ...
                  {(selectedState ? buildersByState[selectedState] : filteredBuilders).length > 0 && (
                    <InfoWindow
                      position={{ lat: mapCenter.lat + 0.05, lng: mapCenter.lng }}
                    >
                      <div>
                        <Typography variant="body2" fontWeight="bold">
                          {(selectedState ? buildersByState[selectedState] : filteredBuilders).length} builders found
                        </Typography>
                      </div>
                    </InfoWindow>
                  )}
