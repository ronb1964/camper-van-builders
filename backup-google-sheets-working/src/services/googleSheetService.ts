...
const convertSheetDataToBuilder = (sheetBuilder: SheetBuilder): Builder => {
  return {
    ...
    rating: sheetBuilder.rating,
    reviewCount: sheetBuilder.reviewCount,
 ...
 */
function parsePhotos(photosInfo: string, builderName: string): string[] {
  ...
  if (photosInfo.includes('photos available')) {
    ...
    for (let i = 1; i <= count; i++) {
      photos.push(`/images/builders/${safeBuilderName}_${i}.jpg`);
    }
    return photos;
  }
      ...
      njBuilders.forEach((builder, index) => {
        ...
        if (!exists) {
          ...
          buildersByState['New Jersey'].push({
            ...
            reviewCount: 50 + Math.floor(Math.random() * 50),
            vanTypes: ['Sprinter', 'Transit', 'ProMaster'],
            priceRange: {
