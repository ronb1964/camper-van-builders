...
import { Builder, Location } from './types';
import { 
  useThemeMode, 
  useRecentlyViewed,
  useScrollToResults
} from './hooks';
import { getBuilders } from './services/googleSheetsService';
  ...
  useEffect(() => {
    const fetchBuilders = async () => {
      try {
        ...
        // Fetch all builders from Google Sheets API
        console.log('🔍 Fetching all builders from Google Sheets...');
        const allBuilders = await getBuilders();
        console.log('🔍 Total builders fetched:', allBuilders.length);
        console.log('🔍 First few builders:', allBuilders.slice(0, 3).map(b => ({ name: b.name, state: b.location.state })));
        ...
        allBuilders.forEach(builder => {
          ...
          console.log(`🔍 Processing builder: ${builder.name} in state: "${state}"`);
        ...
        const stateKeys = Object.keys(buildersByStateData);
        console.log('🔍 States available:', stateKeys);
        stateKeys.forEach(state => {
          console.log(`📊 ${state}: ${buildersByStateData[state]?.length || 0} builders`);
        });
        
        // Specifically log New Jersey builders
        if (buildersByStateData['New Jersey']) {
          console.log(`🔍 NEW JERSEY BUILDERS (${buildersByStateData['New Jersey'].length}):`); 
          buildersByStateData['New Jersey'].forEach((builder, index) => {
